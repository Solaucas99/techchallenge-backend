...
export class ReadQuizSolutionUseCase {
  ...
  public async execute({ id }: IQuiz_Solution): Promise<IQuiz_Solution | void> {
    try {
      this.errors = [];

      const result = await this.quizSolutionsRepository.readQuizSolution({
        id,
      });
