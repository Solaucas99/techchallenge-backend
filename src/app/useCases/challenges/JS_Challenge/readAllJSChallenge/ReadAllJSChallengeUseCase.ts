...
export class ReadAllJSChallengeUseCase {
  ...
  public async execute(): Promise<IJS_Challenge[] | void> {
    try {
      this.errors = [];

      const result = await this.jsChallengesRepository.readAllJSChallenge();
